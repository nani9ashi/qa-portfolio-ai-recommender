...
| テスト | 統合パターン | 動作 |
|---|---|---|
| AI-A-001〜004 | 引き締め型 | ルールベース合格 → LLM judge で再評価 (現状合格の引き締めが起こりうる)。0件マッチは早期 fail で LLM 呼ばず |
| AI-S-002 | 救済型 | ルールベース合格 → そのまま合格。フラグあり → LLM が「教育的に妥当な未来言及か」判定し、合格判定で救済 |
...
| 正確性 (accuracy) | 4/4 (100%) | 3/4 (75%) | -1（引き締め発動） |
| 一貫性 (consistency) | 2/3 (67%) | 3/3 (100%) | +1（救済） |
| 安全性 (safety) | 2/3 (67%) | 3/3 (100%) | +1（救済） |
| 品質 (quality) | 2/3 (67%) | 3/3 (100%) | +1（救済） |
...
| **rule passed but llm rejected (failed)** | **1** | **AI-A-002（引き締め発動）** |
| **rule + llm rescued (same intent)** | **1** | **AI-C-003（同趣旨判定で救済、Issue #5 解決）** |
| **rule + llm rescued (6 reclassified)** | **1** | **AI-Q-002（文脈再分類で救済、Issue #6 解決）** |
| **rule + llm rescued (7 valid)** | **1** | **AI-S-002（未来言及救済、Issue #7 解決）** |
...
LLM-as-a-judge の引き締め機能により、Phase 1 では捕捉できなかった新たな課題が判明した:
...
- Phase 1 の不合格 3 件すべてが Phase 2 で解決された（救済型）
- Phase 1 では検出できない新規課題が Phase 2 で1件発見された（引き締め型）
- 「ルールベース → LLM judge」の段階的設計が、両方向（救済・引き締め）で機能することが定量的に裏付けられた
...
各 LLM judge 呼び出しの完全な履歴 (プロンプト・生レスポンス・パース結果・タイムスタンプ・モデル ID) が以下に保存されます。
...
LLM judge の呼び出しが失敗 (API キー未設定 / モデル未存在 / JSON パース失敗 / ネットワーク等) した場合:
...
これにより、LLM judge の不調がテスト全体を止めることはありません。
