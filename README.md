...
本リポジトリは、QA エンジニアとして取り組んでいる**ポートフォリオシリーズの一つ**です。**戦略的な品質保証・関係者連携・AI プロダクトの品質設計** を中心テーマとしており、ほかのリポジトリとして以下があります。
...
両リポジトリは「**学習の深化の軌跡**」として相互に補完する関係にあります。チケット管理ポートフォリオで CI/CD 構築・E2E 自動化・JSTQB 準拠のドキュメンテーションを基礎として習得した上で、本リポジトリではより戦略的な品質保証・関係者連携・AI プロダクトの品質設計に踏み込みました。
...
- **ルールベース部分**: E2E テストで決定論的に検証、CI で毎 push 自動実行（API コストなし）
- **生成 AI 部分**: 確率的な品質メトリクス（ハルシネーション率・文字数の安定性・教育的妥当性など）で評価、リリース前およびプロンプト変更時に手動実行
...
JSTQB シラバスの枠組み（要求分析 → テスト計画 → テスト設計 → テスト実行）に沿ったプロセス設計、同値分割・境界値分析等のテスト技法など、テストプロセスの専門観点もドキュメント内に組み込んでいます。

特に AI 出力品質テストでは、以下の現代的な評価手法を実装しています:

- **段階的判定設計**: Phase 1（ルールベース）→ Phase 2（LLM-as-a-judge）の二段構造
...
これらは AI 関連プロダクトの QA 実務で重要となる「判定の説明可能性とトレーサビリティ」を組み込んだ設計です。
...
「現場で本当に使えるプロダクトをどう作るか」 に関心を持って学習を続けている社会人です。JSTQB Foundation Level 保有。塾講師や警備員としての経験と、哲学を専攻したバックグラウンドを持ち、**ユーザーや現場目線で仮説を立て、関係者と対話しながら品質を作り込む**スタイルを模索中です。AI プロダクトをはじめとする新しい領域においても、**観察力と論理性**で価値を生み出すビジネスパーソンを目指しています。
