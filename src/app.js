...
(function () {
  ...
  // ===== レコメンド API 呼び出し =====
  // バックエンドの POST /api/recommend を呼び出す。
  // フロントエンド (port 5173) からバックエンド (port 3001) へのクロスオリジン呼び出し。
  // バックエンド側で CORS 許可 + Claude API 経由で推薦理由を生成する。
  const API_ENDPOINT = "http://localhost:3001/api/recommend";

  function callRecommendApi(input) {
    const forceError = $("force-error").checked;
    const forceSlow = $("force-slow").checked;

    // デバッグ用スイッチをクエリパラメータとしてサーバに伝播
    // (現状の app.js の挙動を維持しつつ、E2E テストでは page.route でモックされるため影響なし)
    const params = new URLSearchParams();
    if (forceError) params.append("simulate", "error");
    if (forceSlow)  params.append("simulate", "slow");
